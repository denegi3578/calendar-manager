...
export default function App() {
  ...
  const [showFailed, setShowFailed] = useState(false); // 못함 패널
  const [showDone, setShowDone] = useState(false);     // 완료 패널
  const [showPlan, setShowPlan] = useState(false);
  const [openTaskIds, setOpenTaskIds] = useState({});  // ⋯ 펼침 상태
    ...
    for(const [tid,when] of Object.entries(store.alarms||{})){
      ...
      alarmTimersRef.current[tid]=setTimeout(()=>{
        if("Notification" in window && Notification.permission==="granted"){
          ...
          new Notification(info? `${info.list} · ${info.task.text}`:"할 일 알림",{ body: info?.key && info.key!==UND ? `날짜: ${info.key}`:"미지정" });
        } else alert("할 일 알림 시간입니다!");
      }, t-now);
    }
  ...
  const scheduleAlarm=(list,id,key,dateStr,timeStr)=>{
    if(!timeStr || !/^\d{2}:\d{2}$/.test(timeStr)){ alert("시간(HH:MM)을 입력해 주세요"); return; }
    let dt;
  ...
  const quickAddAt = (ymd) => {
    ...
    if (!list) { alert("목록 인식 실패"); return; }

    const text = window.prompt(`${ymd}에 추가할 할 일을 입력해 주세요`)?.trim();
    if (!text) return;
    ensureKey(ymd);
    const id = makeId("t");
    setStore(p=>{
  ...
  const addList=()=>{
    ...
    if(name === FAIL_LIST){ alert("'못함'은 시스템 목록입니다. 이름으로 쓸 수 없습니다"); return; }
    if(store.lists.order.includes(name)){ alert("이미 있는 목록 이름"); return; }
    setStore(p=>({...p, lists:{order:[...p.lists.order,name], colors:{...p.lists.colors,[name]:nextColor()}}}));
    setNewListName("");
  };
  const removeList=(name)=>{
    if(store.lists.order.length<=1){ alert("최소 1개 목록은 남겨야 합니다"); return; }
    if(name===FAIL_LIST){ alert("'못함'은 시스템 목록입니다."); return; }
    if(!confirm(`'${name}' 목록을 삭제하시겠습니까? (모든 날짜/미지정 포함)`)) return;
    setStore(prev=>{
  ...
  const allByList = useMemo(()=>{
    ...
    for(const [key,obj] of Object.entries(store.dates||{})){
      for(const [list,arr] of Object.entries(obj.tasks||{})){
        ...
        (arr||[])
          .filter(t=>{
           // ✅ deleted 항목 제외 (카드에는 안 보이게)
           if(t.deleted) return false;
           // 완료 안 됐으면 보이기
          if(!t.done) return true;
          // 완료 + 날짜 없음(UND) → 바로 숨김
          if(key===UND) return false;
          // 완료 + 날짜 있음 → 날짜가 오늘/미래면 보이기, 과거면 숨김
    ...
    for(const list of Object.keys(out)){
      out[list].sort((a,b)=>{
        ...
        if (aHas && bHas) {
          ...
          if (aFuture !== bFuture) return aFuture ? -1 : 1; // 미래가 먼저
          if (aFuture && bFuture) {
            if (at !== bt) return at - bt; // 가까운 미래 먼저
            const ta=a.time||"", tb=b.time||""; if(ta!==tb) return ta.localeCompare(tb);
          } else { // 둘 다 과거면 최근(덜 지난) 먼저
  ...
  const removeTask=(list,id,date)=>{
    ...
    if (isRecurring) {
      const choice = window.confirm(
        "반복 할일입니다. 어떻게 삭제하시겠습니까?\n\n" +
        "확인 = 이번만 삭제 (다른 날짜는 계속 생성)\n" +
        "취소 = 반복 중단 (오늘부터 앞으로 쭉 삭제)"
      );
      
      if (choice) {
        // "이번만 삭제" 선택 - 달력에는 남기되 deleted 표시
        setStore(p=>{
        ...
        if (match) {
          ...
          alert("반복이 중단되었습니다. (오늘부터 더 이상 생성되지 않음)");
        }
      }
    } else {
      // 일반 할일 삭제 (기존 로직)
...
// ✅ 중복 생성 완전 차단: ref + meta + localStorage + lock(+TTL)
...
useEffect(() => {
  ...
  setStore(prev => {
    ...
    const recDeleted = { ...(prev.recDeleted || {}) }; // ✅ 삭제 기록 참조
    ...
    return (
      <div style={S.modalOverlay} onClick={() => setQuickAddModal(false)}>
        <div style={S.modalContent} onClick={(e) => e.stopPropagation()}>
          <h3 style={{ margin: '0 0 16px' }}>빠른 추가 (Cmd/Ctrl+K)</h3>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            {/* 날짜 빠른 선택 버튼 */}
            <div>
              <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 6 }}>날짜 선택:</div>
    ...
    return (
      <div style={S.modalOverlay} onClick={() => setRecurringModal(false)}>
        <div style={{...S.modalContent, maxWidth: 600}} onClick={(e) => e.stopPropagation()}>
          <h3 style={{ margin: '0 0 16px' }}>반복 할일 추가</h3>
          
          <div style={{ display: 'grid', gap: 16 }}>
            {/* 목록 선택 */}
            <div>
              <label style={S.label}>목록</label>
              <select
                value={list}
                onChange={(e) => setList(e.target.value)}
                style={S.textInput}
            ...
            <div>
              <label style={S.label}>
                반복 요일 선택 ({weekdays.length}개 선택됨)
              </label>
            ...
            {/* 시간 */}
            <div>
              <label style={S.label}>시간 (선택사항)</label>
              <input
                type="time"
                value={time}
                onChange={(e) => setTime(e.target.value)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                <label style={S.label}>종료일 (선택, 비우면 무제한)</label>
                <input
                  type="date"
                  value={endDate}
            ...
            {weekdays.length > 0 && (
              <div style={{
                ...
                <b>미리보기:</b><br />
                매주 {weekdays.map(d => weekdayButtons[d].label).join(', ')}요일
                {time && ` ${time}`}에<br />
                "{text || '(할일 입력)'}" 생성
              </div>
            )}
    ...
    <div style={S.topNoteBar}>
      <div style={{ display:"flex", alignItems:"center", gap:10, width:"100%" }}>
        ...
        <input
          ...
          placeholder="한 줄로 핵심 (예: 오늘은 미루기 금지)"
        ...
        <div style={S.leftHeader}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
           ...
           <button
             ...
             title={showPlan ? "달력 보기" : "일과표 보기"}
           >
             {showPlan ? "달력" : "일과표"}
           </button>

            <button onClick={gotoPrevMonth} style={S.ghostBtn}>◀</button>
            <div style={S.monthTitle}>{viewYear}년 {viewMonth+1}월</div>
            <button onClick={gotoNextMonth} style={S.ghostBtn}>▶</button>
          </div>
        </div>
        ...
        <div style={S.toolbar}>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <input
              ...
              style={S.listInput}
            />
            <button onClick={addList} style={S.primaryBtn}>목록 만들기</button>
            ...
            <button style={S.secondaryBtn} onClick={()=>setShowDone(v=>!v)}>{showDone?"작업 보기":"완료 보기"}</button>
            <button style={S.secondaryBtn} onClick={()=>setShowFailed(v=>!v)}>{showFailed?"작업 보기":"못함 보기"}</button>
            <button style={S.secondaryBtn} onClick={()=>setShowTrash(v=>!v)}>{showTrash?"작업 보기":"휴지통 보기"}</button>
          </div>
        </div>
                ...
                <div key={list} style={{...S.card, background: `linear-gradient(180deg, #ffffff 0%, #fafbff 100%)`, borderColor: colorOf(list) }}
                  ...
                  <div style={S.cardHead}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <input type="color" value={colorOf(list)} onChange={(e)=>setListColor(list,e.target.value)} style={S.colorPicker} title="색 변경"/>
                    </div>
                    <div style={{color:"#6b7280",fontSize:12}}>완료 {doneCnt}/{items.length}</div>
                  </div>

                  {/* 추가 입력 */}
                  <div style={S.addRow}>
                    <input style={S.textInput} placeholder={`${list}에 할 일 (날짜 없어도 ok)`}
                    ...
                    <input type="time" style={S.timeInput}
                           ...
                           onKeyDown={(e)=>e.key==="Enter" && addTaskGlobal(list)} />
                    <button style={S.primaryBtn} onClick={()=>addTaskGlobal(list)}>추가</button>
                  </div>
...
>
  ...
  >
    {t.text}
                        ...
                        <div style={S.ctrlCol}>
                          <button
                            style={S.ghostBtn}
                            title="자세히(우선/미루기/알람/하위추가)"
                            onClick={()=>setOpenTaskIds(p=>({...p,[t.id]:!p[t.id]}))}
                          >
                            ⋯
                          </button>
                          <button style={S.dangerBtn} onClick={()=>removeTask(list,t.id,t.date)}>삭제</button>
                        </div>
                          ...
                          <ul style={S.subList}>
                            {(t.subtasks||[]).map(st=>(
                              <li key={st.id} style={S.subItem}>
                                ...
                                <button style={S.smallBtn} onClick={()=>removeSubtask(list,t.id,t.date,st.id)}>삭제</button>
                              </li>
                            ))}
                          </ul>
                        )}

                        {/* 펼침 패널: 우선/미루기/알람 + 하위추가 입력만 */}
                        {openTaskIds[t.id] && (
                          <div style={S.expandBox}>
                            ...
                            <div style={{display:"flex",gap:6, marginTop:8, paddingLeft:18}}>
                              <input id={`sub_${t.id}`} placeholder="하위 할일 내용" style={S.subAddInput}
                                     onKeyDown={(e)=>{
                                       if(e.key==="Enter"){
                    ...
                    {items.length===0 && <li style={S.empty}>할 일이 없어</li>}
                  </ul>

                  <div style={{display:"flex",justifyContent:"flex-end",marginTop:10}}>
                    <button style={S.deleteBtn} onClick={()=>removeList(list)}>목록 삭제</button>
                  </div>
                </div>
              );
            })}
                ...
                <li key={item.id} style={S.item}>
                  <div style={{display:"flex",flexDirection:"column",gap:6, minWidth:0}}>
                    ...
                    <div style={{color:"#6b7280",fontSize:12}}>
                      목록: {item.list} · 날짜: {item.key===UND?"미지정":item.key}{item.task.time?` ${item.task.time}`:""}
                    </div>
                  </div>
                  <div style={S.ctrlCol}>
                    <button style={S.secondaryBtn} onClick={()=>markUndone(item.list, item.key, item.id)}>미완료로</button>
                    <button style={S.dangerBtn} onClick={()=>removeTask(item.list, item.id, item.key)}>삭제</button>
                  </div>
                </li>
              ))}
              {getAllDone().length===0 && <li style={S.empty}>완료된 항목 없음</li>}
            </ul>
          </div>
        ) : showFailed ? (
          /* 못함 패널 */
          <div style={S.trashPanel}>
            <h3 style={{margin:0}}>못함</h3>
            <ul style={S.list}>
              {getAllFailed().map(item=>(
                <li key={item.id} style={S.item}>
                  <div style={{display:"flex",flexDirection:"column",gap:6, minWidth:0}}>
                    <b style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{item.task.text}</b>
                    <div style={{color:"#6b7280",fontSize:12}}>
                      날짜: {item.key===UND?"미지정":item.key}
                      {item.task.time?` ${item.task.time}`:""} · 등록: {new Date(item.task.failedAt).toLocaleString()}
                    </div>
                  </div>
                  <div style={S.ctrlCol}>
                    <button style={S.secondaryBtn} onClick={()=>restoreFailed(item.key, item.id)}>다른 목록으로</button>
                    <button style={S.smallBtn} title="+1일" onClick={()=>snoozeFailed(item.key, item.id, 1)}>+1d</button>
                    <button style={S.dangerBtn} onClick={()=>deleteFailed(item.key, item.id)}>삭제</button>
                  </div>
                </li>
              ))}
              {getAllFailed().length===0 && <li style={S.empty}>못한 항목 없음</li>}
            </ul>
          </div>
        ) : (
          /* 휴지통 패널 */
              ...
              {(store.trash||[]).map(tr=>(
                <li key={tr.id} style={S.item}>
                  <div style={{display:"flex",flexDirection:"column",gap:6, minWidth:0}}>
                    ...
                    <div style={{color:"#6b7280",fontSize:12}}>목록: {tr.list} · 날짜: {tr.key===UND?"미지정":tr.key} · 삭제: {new Date(tr.deletedAt).toLocaleString()}</div>
                  </div>
                  <div style={S.ctrlCol}>
                    <button style={S.secondaryBtn} onClick={()=>restoreTrash(tr.id)}>되돌리기</button>
                    <button style={S.dangerBtn} onClick={()=>deleteForever(tr.id)}>완전삭제</button>
                  </div>
                </li>
              ))}
              {(store.trash||[]).length===0 && <li style={S.empty}>휴지통이 비어있습니다</li>}
            </ul>
          </div>
        )}
      </div>
  ...
  function restoreFailed(dateKey, taskId){
    ...
    if(!lists.length) return alert("목록이 없습니다");
    const list = prompt(`복구할 목록?\n${lists.join(", ")}`, lists[0])?.trim();
  ...
  return Object.entries(dd.tasks || {})
    .flatMap(([list, arr]) =>
      (arr || [])
        .filter(t => {
          if (list === FAIL_LIST) return includeFailed; // ✅ 못함은 옵션일 때만
          return true;
        })
        .map(t => ({
          text: t.text,
          ...
          deleted: t.deleted,
          failed: list === FAIL_LIST || !!t.failed, // ✅ 표시용 플래그
        }))
    );
}
  ...
  return (
    <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"0 0 8px"}}>
        <div style={{fontWeight:800}}>{ymd} 일과표</div>
      </div>
        ...
        {blocks.map(b=>{
          ...
          return (
            <div key={b.id} style={{
              ...
              {!b.fromRecurring && <button style={S.smallBtn} onClick={()=>removeBlock(b.id)}>삭제</button>}
            </div>
          );
        })}
      </div>
...
function PlanPanel({ ymd, store, setStore, colorOf, lists, saveDetails }){
  ...
  const [recForm,setRecForm]=useState({text:"",days:"월,화,수,목,금",startTime:"22:30",endTime:"06:30",color:"#334155",start:ymd,end:""});

  return (
    <div style={S.planRight}>
      <div style={{display:"grid",gap:10}}>
        <div style={{fontWeight:800}}>{ymd} 할일 배치</div>
        ...
        <div style={{fontWeight:800}}>새 블록(일정)</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 100px 100px 90px auto",gap:8}}>
          <input placeholder="제목" value={newBlk.text} onChange={e=>setNewBlk({...newBlk, text:e.target.value})} style={S.textInput}/>
          ...
          <button style={S.primaryBtn} onClick={()=>{
            const {text,start,end}=newBlk; if(!text.trim()||!isHHMM(start)||!isHHMM(end)) return alert("입력 확인");
        ...
        <div style={{fontWeight:800}}>반복 블록(요일)</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 100px 100px 120px 120px 90px auto",gap:8}}>
          <input placeholder="예: 수면" value={recForm.text} onChange={e=>setRecForm({...recForm, text:e.target.value})} style={S.textInput}/>
          ...
          <input placeholder="요일(월,화…)" value={recForm.days} onChange={e=>setRecForm({...recForm, days:e.target.value})} style={S.textInput}/>
          <input placeholder="시작일" value={recForm.start} onChange={e=>setRecForm({...recForm, start:e.target.value})} style={S.textInput}/>
          <input placeholder="종료일(선택)" value={recForm.end} onChange={e=>setRecForm({...recForm, end:e.target.value})} style={S.textInput}/>
          <button style={S.secondaryBtn} onClick={()=>{
            const byweekday = recForm.days.replace(/\s/g,"").split(",").map(k=>WEEKDAY_MAP[k]).map(Number).filter(Number.isFinite);
            if(!recForm.text.trim()||!isHHMM(recForm.startTime)||!isHHMM(recForm.endTime)||byweekday.length===0) return alert("입력 확인");
